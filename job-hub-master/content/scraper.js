...
(() => {
  ...
  const matchBody = (re) => (bodyText().match(re) || [,''])[1]?.trim() || '';
  const longText = (selectors) => {
    for (const sel of selectors.split(',')) {
      const value = document.querySelector(sel.trim())?.textContent?.trim().replace(/\s+/g, ' ');
      if (value && value.length > 30) return value.slice(0, 12000);
    }
    return '';
  };
  ...
  return {
    ...
    position,
    jobName: position,
    jobId: matchBody(/(?:岗位编号|职位编号|职位ID|Job\s*ID)\s*[：:#]?\s*([\w-]+)/i),
    workLocation: matchBody(/(?:工作地点|工作地址|办公地点|地点)\s*[：:]?\s*([^\n]{2,40})/i),
    jd: longText('[class*="job-description"], [class*="jobDescription"], [class*="job-detail"], [class*="jobDetail"], [class*="requirement"], [class*="description"]'),
    url: location.href,
    officialUrl: location.href,
