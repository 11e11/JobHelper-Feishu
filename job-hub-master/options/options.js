...
const MAP_KEYS = Object.keys(DEFAULT_FIELD_MAP);
...
async function init() {
  ...
  $('btnClearCredentials')?.addEventListener('click', onClearCredentials);
...
async function onClearCredentials() {
  await chrome.runtime.sendMessage({ type: 'JT_CLEAR_CREDENTIALS' });
  $('cAppSecret').value = '';
  showMsg('configMsg', '✓ 已清除 App Secret 和访问令牌', 'success');
}
...
async function onTestConnection() {
  ...
  try {
    ...
    if (resp && resp.ok) {
      ...
      if (resp.optionalMissing && resp.optionalMissing.length) {
        text += `\n\nℹ 未建的可选字段将自动跳过：${resp.optionalMissing.join('、')}`;
      }
      ...
      let text = (resp && resp.error) || '测试失败';
      if (resp?.fields?.length) text += `\n\nAPI 实际返回字段：\n${resp.fields.map(f => `· ${f.name}（${f.type}）`).join('\n')}`;
      showMsg('configMsg', text, 'error');
    }
  } catch (err) {
