...
export function isAiEnabled(config) {
  void config;
  return false; // 阶段一硬闸门；用户验收前不可由存储配置解除。
}

// ---------- Core API call ----------

export async function callAI({ systemPrompt, userMessage, temperature = 0.1, jsonMode = true }) {
  void systemPrompt; void userMessage; void temperature; void jsonMode;
  throw new Error('阶段一安全模式：LLM 网络调用未启用');
  /* 阶段三启用真实提供商时，必须由显式配置与域名白名单保护。
...
export async function testAiConnection(config) {
  void config;
  return { ok: false, error: '阶段一安全模式：LLM 网络调用未启用' };
  /* 阶段三启用后再恢复连接测试。
  const provider = AI_PROVIDERS[config.provider];
