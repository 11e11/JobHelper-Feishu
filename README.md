...
- `auto`：默认用于所有已识别且资料中有值的普通字段，包括身份证、期望城市、薪资、调剂等；置信度必须至少为 0.8。
- `confirm`：用户可在设置页把任意可配置字段改为每次确认；此类字段绝不批量填写，只在单项点击并再次确认后填写。
- `forbidden`：用户可禁止任意资料字段。密码、验证码、银行信息、签名、诚信声明、文件/简历上传和提交控件属于不可解除的硬禁止项。
